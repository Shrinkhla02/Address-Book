...
import com.address.address_management.model.dto.country.USAAddressDTO;
...
@Component
public class USAddressFactory implements AddressHandler {
    
    
    private final AddressRepository addressRepository;

    @Autowired
    public USAddressFactory(AddressRepository addressRepository) {
    ...
    public AddressValidationResponseDTO validate(AddressRequestDTO address) {
        ...
        if (!isValidUSZipCode(address.getZipcode())) {
            errors.add(ValidationError.builder()
                    .field("zipcode")
                    .code("INVALID_ZIPCODE")
                    .message("Invalid US ZIP code format. Expected format: 5 digits or ZIP+4 (12345 or 12345-6789)")
                    .build());
        }
